...
<body>
	...
	<form method="post" id="toevoegform">
		<label>Naam:<span>${fouten.naam}</span> <input name="naam"
			...
			type="number" min="0.01" step="0.01"></label>
		<div>
			<label> <input type="radio" name="soort" value="F" id="food"
				${param.soort=="F" ? "checked" : ""}> Food <span>${fouten.soort}</span>
			</label>
		</div>
		<label>Houdbaarheid:<span>${fouten.houdbaarheid}</span> <input
			name="houdbaarheid" value="${param.houdbaarheid}" type="number"
			id="houdbaarheid" min="1">
		</label>
		<div>
			<label> <input type="radio" name="soort" value="NF"
				id="nonfood" ${param.soort=="NF" ? "checked" : ""}> Non-Food
			</label>
		</div>
		<label>Garantie:<span>${fouten.garantie}</span> <input
			name="garantie" value="${param.garantie}" type="number" id="garantie"
			min="0">
		</label> 
